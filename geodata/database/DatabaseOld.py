...
class Database:
    ...
    def __init__(self, path):
        ...
        column_defs = list(map(columns, lambda x: x + ' text'))
        column_defs[0] = column_defs[0] + ' PRIMARY KEY'

        self.c.execute('''CREATE TABLE column_headers
                          (?)''', ', '.join(column_defs))

        with open(self.path + 'ACS_5yr_Seq_Table_Number_Lookup.csv','rb') as f:
            rows = csv.reader(f) # comma is default delimiter
            print(list(rows)[:5])
